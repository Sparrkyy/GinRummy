...
func loginEndpoint(c *gin.Context) {
    ...
    if err != nil { fmt.Println(gin.H{"error": err.Error()}); c.JSON(http.StatusBadRequest, "Authenication Failed"); return }
	defer rows.Close()
    for rows.Next() {
        ...
        if CheckPasswordHash(input.Password, hashedpassword) {
            ...
            if err != nil { c.JSON(http.StatusBadRequest, "Authenication Failed: Wrong Password"); return}
            c.JSON(http.StatusOK, jwtResponse{ JWT: newJWT } )
            return
        } else {
            c.JSON(http.StatusBadRequest, "Authenication Failed: Wrong Password")
            return
        }
    }
}
